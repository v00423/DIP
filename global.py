...
def apply_transform(image, scale, rotation, translation_x, translation_y, flip_horizontal):
    ...
    # 构建围绕中心的缩放 + 旋转矩阵
    M_scale_rot = cv2.getRotationMatrix2D(center, rotation, scale)#返回一个 2x3 的仿射变换矩阵，包含了旋转和缩放的组合效果
    ...
    # 先缩放+旋转，再平移实现组合变换
    M = M_translate @ to_3x3(M_scale_rot)
    M = M[:2]  # 变换为 2×3 矩阵给 warpAffine 使用
