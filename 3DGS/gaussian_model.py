...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        # Use log space for unconstrained optimization
        mean_dists = torch.mean(torch.sqrt(dists[0]), dim=1, keepdim=True) * 2.0
        mean_dists = mean_dists.clamp(
            0.2 * torch.median(mean_dists),
            3.0 * torch.median(mean_dists)
        )  # Prevent infinite scales
        print('init_scales', torch.min(mean_dists), torch.max(mean_dists))
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        # Compute covariance:
        # Typical 3DGS form: Sigma = R * (S S^T) * R^T
        # Here S is diagonal so S S^T = diag(s^2).
        Covs3d = torch.bmm(R, torch.bmm(torch.bmm(S, S.transpose(1, 2)), R.transpose(1, 2)))  # (N,3,3)

        # Small epsilon to keep numerical SPD during inversion/projection
        eps = 1e-6
        Covs3d = Covs3d + eps * torch.eye(3, device=Covs3d.device, dtype=Covs3d.dtype).unsqueeze(0)
